...
export default function JobDetailsScreen() {
  ...
  const handleApply = async () => {
    try {
      ...
      Toast.show({ type: "success", text1: "Application submitted and sent to the employer via chat." });
      setHasApplied(true);
          ...
          {user?.role === "freelancer" && (
            <>
              ...
              <Button
                ...
                disabled={isSaving}
                icon={<MaterialIcons name={isSaved ? "bookmark" : "bookmark-border"} size={18} color={isSaved ? colors.text : colors.purple} />}
                style={{ width: "100%" }}
              />
            </>
          )}
